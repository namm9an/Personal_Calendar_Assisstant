...
class TestMongoDBModels:
    ...
    async def test_event_validation(self, test_db):
        ...
        with pytest.raises(ValidationError):
            await test_db.events.insert_one(invalid_event)
    ...
    async def test_user_validation(self, test_db):
        ...
        with pytest.raises(ValidationError):
            await test_db.users.insert_one(invalid_user)

    async def test_session_validation(self, test_db):
        """Test session validation rules."""
        # Test invalid session (missing required fields)
        invalid_session = {
            "_id": ObjectId(),
            "user_id": str(ObjectId()),
            "provider": "google",
        ...
        with pytest.raises(ValidationError):
            await test_db.sessions.insert_one(invalid_session) 